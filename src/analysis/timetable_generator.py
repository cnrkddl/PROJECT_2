...
MIN_CONDITIONS       = 1     # 3조건 중 최소 충족 개수 (미달 시 타임테이블 제외)
...
class AdTimetableGenerator:
    ...
    def generate_timetable(self,
        ...
        # ── 씬 타임스탬프 로드 (조건 2: 씬 전환 시각 목록 + 절대 시간 보정용) ─
        # scene_start_map: {씬이름 → 씬 시작 절대 시간(초)}
        # YOLO 결과의 등장 시간이 씬 상대 시간으로 저장된 경우 절대 시간으로 보정에 사용
        scene_start_map = {}
        scene_start_times = []
        if os.path.exists(scene_timestamps_csv):
            with open(scene_timestamps_csv, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    try:
                        start = float(row.get('시작 시간 (초)', 0))
                        scene_start_map[row.get('씬 이름', '')] = start
        ...
        for i, scene in enumerate(scenes):
            ...
            relative_sec    = float(scene.get('등장 시간 (초)', 0))
            object_density  = float(scene.get('오브젝트 밀도', 1.0))
            scene_desc      = scene.get('장면 설명', '')

            # 씬 번호 추출
            _m = re.search(r'scene-0*(\d+)', scene_name)
            scene_num = int(_m.group(1)) if _m else 0

            # 씬 번호 1~999: run_pipeline의 03d 포맷 불일치로 scene_start_sec=0 적용됨
            #                  → 상대 시간이므로 씬 절대 시작 시간을 더해 보정
            # 씬 번호 1000+:  03d 포맷이 4자리와 일치하여 절대 시간이 이미 저장됨
            #                  → 그대로 사용
            scene_abs_start = scene_start_map.get(scene_name, 0.0)
            if scene_num < 1000:
                timestamp_sec = round(scene_abs_start + relative_sec, 2)
            else:
                timestamp_sec = round(relative_sec, 2)
            ...
            # Gemini가 의미 있는 설명을 못 만든 fallback 씬 제외
            # ("화면에 ~이(가) 등장하는 장면" 패턴 → 임베딩 품질 낮아 우연 매칭 위험)
            if re.search(r'화면에 .+이\(가\) 등장하는 장면', scene_desc):
                print(f"  [SKIP] {scene_name} — fallback 설명, 매칭 제외")
                continue
