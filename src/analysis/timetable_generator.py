...
class DynamicTimetableGenerator:
    ...
    def generate_timetable(self, transcript_csv_path: str, vision_csv_path: str, output_csv_path: str):
        ...
        if os.path.exists(vision_csv_path):
            with open(vision_csv_path, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    start_sec = row.get('시작 시간 (초)', '')
                    end_sec = row.get('종료 시간 (초)', '')
                    # 실제 시간(초)이 있으면 정확한 타임라인 표시, 없으면 씬 이름으로 폴백
                    time_label = f"{start_sec}s ~ {end_sec}s" if start_sec and end_sec else scene
                    vision_lines.append(f"[{time_label}] 화면에 등장한 주요 사물: {color} 색상의 {material} 재질로 된 {style} {obj}")
        ...
        아래는 어떤 드라마/영상의 [시각(Vision) 객체 데이터]와 [대사 데이터]입니다.

        [분석 우선순위 및 요청 사항]
        1. **1순위 - 화면 사물(Vision) 기반 광고 매칭**: 시각 데이터에서 화면에 등장한 사물(의자, 가방, 노트북 등)과 그 시간대를 파악하세요. 사물이 등장하는 해당 시간대를 광고 후보 구간으로 우선 선정하세요. (사물과 직접 연결되는 상품 광고 = 가장 자연스러운 PPL 효과)
        2. **2순위 - 대사(Audio)로 맥락 검증**: 1순위에서 선정한 시간대의 대사 흐름을 확인하여, 분위기가 광고를 노출하기 적합한지 (반전 없는 잔잔한 구간, 대화 공백 구간 등) 검증/조정해 주세요.
        3. **광고 노출 시간**: 광고를 띄우기 가장 임팩트 있는 '진입 시점'부터 **딱 5초~10초 동안만** 노출하도록 start_time/end_time을 설정해 주세요.
        4. **광고 상품 선정**: 화면 사물과 직접 연결되는 상품을 1순위로 추천하고, 사물 정보가 없는 구간은 대사 흐름에서 유추한 분위기 기반 상품을 추천하세요.
        5. 각 광고의 맥락(예: 갈등, 로맨스, 오피스 등)을 정의하고, **화면에 등장한 사물이 무엇인지를 항상 reason에 명시**해 주세요.
        ...
        [
            {{
                ...
                "reason": "화면에 등장한 사물: [사물명], 광고 추천 사유"
            }},
            ...
        ]

        ============= 영상 분석 데이터 =============
        [부분 1: 시각(Vision) 씬별 등장 사물 데이터 ← 광고 매칭 1순위]
        {full_vision}

        [부분 2: 청각(대사) 데이터 ← 맥락 검증 2순위]
        {full_transcript}
        ============= 데이터 끝 =============
        """
