...
class DynamicTimetableGenerator:
    ...
    def generate_timetable(self, transcript_csv_path: str, vision_csv_path: str, output_csv_path: str):
        ...
        [요청 사항]
        1. 대사의 흐름을 분석하여 의미 있는 내용/분위기 전환 구간을 파악해 주세요.
        2. 해당 구간 안에서, 광고를 띄우기 가장 임팩트 있는 **'진입 시점'부터 딱 5초~10초 동안만** 광고를 노출하도록 시간을 설정해 주세요. (예: 어떤 분위기가 20초간 지속되어도, 광고 노출 시간은 5초만 설정)
        3. 해당 구간의 씬(Scene)에서 화면에 등장한 사물(Vision 데이터)이 있다면, 단순히 분위기뿐만 아니라 화면에 노출된 사물(의자, 가방 등)과 직접 연결되는 광고 상품을 우선적으로 추천해 주세요. (가장 자연스러운 PPL 효과 달성)
        4. 각 광고의 맥락(예: 갈등, 로맨스, 오피스 등)을 정의하고, 시각 정보와 대사 흐름이 절묘하게 어우러지는 기발한 광고 상품을 1개씩 추천해 주세요.
